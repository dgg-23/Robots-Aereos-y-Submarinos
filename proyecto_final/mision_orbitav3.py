...
def main():
    ...
    with open(csv_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["ID", "Filename", "Lat", "Lon", "Alt", "Heading", "AltObjetivo", "OK"])

        for i, (lat_wp, lon_wp, alt_wp) in enumerate(puntos, start=1):
            # 1) Ir al waypoint
            ir_a(drone, lat_wp, lon_wp, alt_wp)

            # 2) Esperar a llegar y PARAR (quieto de verdad)
            _ = esperar_llegada_y_parada(drone, lat_wp, lon_wp, timeout_s=40)
            time.sleep(TIEMPO_ESTABILIZACION_S)

            # 3) Calcular yaw hacia el centro usando estado actual
            st = obtener_estado(drone)
            if st is None:
                curr_lat, curr_lon, curr_alt, curr_hdg, gs, climb = 0, 0, 0, 0, 0, 0
            else:
                curr_lat, curr_lon, curr_alt, curr_hdg, gs, climb = st

            bearing = calcular_bearing(curr_lat, curr_lon, CENTRO_LAT, CENTRO_LON)

            # 4) Girar al centro y esperar a estar orientado (y quieto)
            yaw_hacia(drone, bearing)
            time.sleep(ESPERA_YAW_S)
            _ = esperar_yaw_al_centro(drone, bearing, timeout_s=12)

            # 5) Reafirmar ROI + gimbal y esperar un poco
            apuntar_roi(drone, CENTRO_LAT, CENTRO_LON, ROI_ALT)
            if USAR_GIMBAL_PITCH:
                set_gimbal_pitch(drone, GIMBAL_PITCH_DEG)
            ...
            # 6) Foto (quieto y mirando al centro)
            nombre_archivo = f"foto_{i:03d}.jpg"
            ruta_foto = os.path.join(carpeta_dataset, nombre_archivo)
            ok = capturar_foto_ffmpeg(ruta_foto, max_reintentos=6, timeout_s=8)

            # 7) Espera 1 segundo tras la foto (como pediste)
            time.sleep(ESPERA_POST_FOTO_S)

            # 8) Registrar
            writer.writerow([i, nombre_archivo, curr_lat, curr_lon, curr_alt, bearing, alt_wp, int(ok)])

            if ok:
                print(f"[FOTO {i}/{len(puntos)}] OK (parado + cara al centro) alt={alt_wp} yaw={bearing:.1f}°")
            else:
                print(f"[FOTO {i}/{len(puntos)}] ERROR capturando frame (se registra igualmente).")
